	...
	<div class="container">
		<h3>
			<a href="${pageContext.request.contextPath}/"
				style="text-decoration: none;">PHONE-STORE</a>
		</h3>
		<hr>
		<div class="row">
				...
				<form role="search"
					...
					<div class="input-group">
						<input type="text" class="form-control"
							placeholder="Enter keywork here..." name="key"
							value="<%=keyword%>">
									...
									<c:if test="${!empty phone.name}">
										<div class="form-group">
											<input type="submit" class="btn btn-primary"
												value="<spring:message text="Update Phone"/>" /> <a
												href="<c:url value='/phones' />" class="btn btn-info">Back</a>
										</div>
									</c:if>
									<c:if test="${empty phone.name}">
										<div class="form-group">
			...
			<c:forEach items="${listPhones}" var="phone">
				<tr>
					...
					<td class="text-center"><a
						href="<c:url value='/edit/${phone.id}' />"
						class="btn btn-sm btn-primary">Update</a> <a
						href="<c:url value='/remove/${phone.id}' />"
		...
		<c:if test="${count > 0}">
			...
			uri="${pageContext.request.contextPath}/phones" next="&raquo;"
			previous="&laquo;" />
		</c:if>
